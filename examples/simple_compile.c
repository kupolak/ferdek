...
int main() {
    print_value(make_string("=== Compiler Test ==="));
    FerdekValue x = make_int(10);
    FerdekValue y = make_int(5);
    FerdekValue sum = make_int(0);
    sum = make_int(to_int(x) + to_int(y));
    print_value(make_string("Sum:"));
    print_value(sum);
    FerdekValue i = make_int(5);
    while (to_bool(make_bool(to_int(i) > to_int(make_int(0))))) {
        print_value(i);
        i = make_int(to_int(i) - to_int(make_int(1)));
    }
    print_value(make_string("Done!"));
    return 0;
}
